...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $step === 'request') {
    ...
    if (empty($username) || empty($membership_number)) {
        $message = "الرجاء إدخال اسم المستخدم ورقم العضوية";
    } else {
        // التحقق من صحة البيانات
        $stmt = $pdo->prepare("SELECT id, full_name, username, membership_number FROM persons WHERE username = ? AND membership_number = ? LIMIT 1");
        $stmt->execute([$username, $membership_number]);
        $member = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if ($member) {
            // حفظ في الجلسة للخطوة التالية
            $_SESSION['reset_member_id'] = $member['id'];
            $_SESSION['reset_member_username'] = $member['username'];
            header("Location: forgot_password.php?step=reset");
            exit();
        } else {
            $message = "اسم المستخدم أو رقم العضوية غير صحيح";
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $step === 'reset') {
    ...
    if (empty($new_password) || empty($confirm_password)) {
    ...
    } elseif (!isset($_SESSION['reset_member_id'])) {
        ...
        $updateStmt->execute([$passwordHash, $_SESSION['reset_member_id']]);
        
        // حذف بيانات الجلسة
        unset($_SESSION['reset_member_id']);
        unset($_SESSION['reset_member_username']);
...
<div class="wrapper">
    <div class="glass-box">
        <?php if ($step === 'request'): ?>
            ...
            <div class="tagline">
                أدخل اسم المستخدم ورقم العضوية للتحقق من هويتك
            </div>
            ...
            <form method="POST">
                ...
                <input type="text" name="membership_number" placeholder="رقم العضوية" required>
        ...
        <?php elseif ($step === 'reset'): ?>
            <?php if (!isset($_SESSION['reset_member_id'])): ?>
                <h2>انتهت صلاحية الجلسة</h2>
                <div class="error">يرجى المحاولة مرة أخرى من البداية</div>
                <a href="forgot_password.php" style="text-decoration:none;display:block;">
